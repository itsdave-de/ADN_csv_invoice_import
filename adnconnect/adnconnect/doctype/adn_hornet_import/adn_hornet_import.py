...
class ADNHornetImport(Document):
	...
	def do_import(self):
		...
		rechnungsdatum = datetime.strptime(rechnung["datum"],"%d.%m.%Y %H:%M")   
		...
		if self.validate_csv(erste_zeile):
			...
			for row, pos in df.iterrows():
				...
				if pos['Endkunde'] != kunde:
					...
					rechnung = {}
					customer = frappe.get_all("Customer", filters = {"hornet_domain": pos['Endkunde_Reference']}, fields = ["name","customer_name"])
					print(customer)
					if len(customer)== 1:
					...
					rechnung["kdnr"] =  cust
					rechnung["adn_rg"] = str(pos['RECHNUNG'])+ cust
	...
	def create_erpn_invoice(self, rechnungen):
		...
		rechnungsdatum = datetime.strptime(rechnungen["datum"],"%d.%m.%Y %H:%M")
